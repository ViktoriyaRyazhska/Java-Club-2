...
public class TestTask33 {
	...
	private static class DinglemouseAnswer20180123 {
		static int[] humanYearsCatYearsDogYears(final int h) {
			return new int[] { h, h == 1 ? 15 : h == 2 ? 24 : 24 + 4 * (h - 2),
	...
	public void random() {
		for (int r = 0; r < 100; r++) {
			...
			final int[] actual = Task33.humanYearsCatYearsDogYears(humanYears);
			System.out.println(String.format("Random test %d: human years %d => %s", r + 1, humanYears,
